...
sudo yum update -y 
sudo yum install -y python-minimal   \
                    python-pip       \
                    python-devel     \
                    libffi-devel     \
                    gcc              \
...
sudo sed -i 's/#enable_horizon_magnum: "{{ enable_magnum | bool }}"/enable_horizon_magnum: "{{ enable_magnum | bool }}"/g' /etc/kolla/globals.yml && \
...
sudo sed -i 's/#kolla_internal_fqdn: "{{ kolla_internal_vip_address }}"/kolla_internal_fqdn: "kolla.marthy.xyz"/g' /etc/kolla/globals.yml && \
sudo sed -i 's/#kolla_external_fqdn: "{{ kolla_external_vip_address }}"/kolla_external_fqdn: "openstack.marthy.xyz"/g' /etc/kolla/globals.yml && \
##sudo sed -i 's///g' /etc/kolla/globals.yml && \
sudo kolla-genpwd && \
sudo kolla-ansible -i ./all-in-one bootstrap-servers && \
sudo kolla-ansible pull -i ./all-in-one && \
...
sudo chown `whoami`:`whoami` ./admin-openrc.sh && \
